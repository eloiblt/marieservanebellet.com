...
router.delete('/:id', authenticateJWT, (req, res) => {
  PictureCollection.findOne({ id: req.params.id }).lean()
    .then(doc => {
      ...
      fs.unlink('/var/www/marieservanebellet.com/pictures/' + (picture as Picture).url, function (err) {
...
router.post('/postFile', authenticateJWT, (req: any, res) => {
  if (process.env.NODE_ENV === "development") {
    ...
    sampleFile.mv('/var/www/marieservanebellet.com/pictures/' + sampleFile.name, err => {
      if (err) throw err;
      exec('jpegoptim --max=90 /var/www/marieservanebellet.com/pictures/' + sampleFile.name, (err, stdout, stderr) => {
        if (err) {
          console.error(err)
        }
